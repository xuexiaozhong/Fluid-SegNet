...
class DoubleConv_2d(nn.Sequential):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        super(DoubleConv_2d, self).__init__(
            ...
            nn.Conv2d(mid_channels, out_channels, kernel_size=3, stride=1, padding=2, bias=False, dilation=2),
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        x0 = x
        x0 = self.channel_attention(x) * x0
        out = self.spatial_attention(x) * x0
        return out
...
class CBAM_Dilated_Y_Net(nn.Module):
    def __init__(self,
        ...
        super(CBAM_Dilated_Y_Net, self).__init__()
