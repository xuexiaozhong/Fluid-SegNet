...
class DS_dilatedConv(nn.Module):
    def __init__(self, in_channels):
        super(DS_dilatedConv, self).__init__()
        self.DownS = nn.MaxPool2d(2, stride=2)
        self.Dilated_conv = nn.Conv2d(in_channels=in_channels, out_channels=in_channels, kernel_size=3, stride=1, padding=2, dilation=2)
        self.relu = nn.ReLU()
        self.Ups = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=2, stride=2)
...
class Fusion(nn.Module):
    def __init__(self, channels):
        ...
        self.ReLu3d = nn.ReLU()
        self.ds_dilated_conv = DS_dilatedConv(channels)
        self.ReLuds = nn.ReLU()

    def forward(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor:
        x_2d = self.conv2d(x1)
        x_3d = self.conv3d(x2)
        x_ds = self.ds_dilated_conv(x1)

        return self.ReLu2d(x_2d) + self.ReLu3d(x_3d) + self.ReLuds(x_ds)
...
class DS_DC_YNet(nn.Module):
    def __init__(self,
        ...
        super(DS_DC_YNet, self).__init__()
